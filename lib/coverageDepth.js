...
// maxContributors: caps the per-cell contributor list stored in the bulk
// grid response (kept small so the whole-grid payload/cache stays sane on a
// Pi-class host across thousands of cells) — pass null for the full list,
// used by the single-cell detail endpoint where only one cell is ever computed.
function computeDivisionDepth(cells, bidders, divCfg, maxContributors = 8) {
  const result = {};
  for (const cell of cells) {
    ...
    result[cell] = {
      ...
      contributors: maxContributors == null ? contributors : contributors.slice(0, maxContributors)
    };
  }
  return result;
}
