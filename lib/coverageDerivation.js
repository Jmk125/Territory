...
// edge) is <= +1%. Returns { edge, bandN } or null if none qualifies.
function findCompetitiveRadius(obs, edges, distField) {
  let best = null;
  for (const edge of edges) {
    const within = obs.filter(o => o[distField] != null && o[distField] <= edge);
    if (!within.length) continue;
    const med = median(within.map(o => o.dev_pct));
    if (med <= 1.0) best = { edge, bandN: within.length };
  }
  return best;
}

// First edge beyond which median dev > +5% OR win rate < 10%.
function findOuterRadius(obs, edges, distField) {
  for (const edge of edges) {
    ...
    if (med > 5 || winRate < 0.10) return { edge, bandN: beyond.length };
  }
  return null;
}

function radiiForUnit(obs, edges, distField, tier, fallbackPair) {
  let competitive = findCompetitiveRadius(obs, edges, distField);
  let outer = findOuterRadius(obs, edges, distField);
...
function bidderAutoFlags(bidderName, obs, competitiveMi, outerMi) {
  ...
  const own = findCompetitiveRadius(obs, MILE_EDGES, 'miles');
...
function deriveConfig({ observations, divisionNames = {}, manualOverrides = {}, previousConfig = null }) {
  ...
  for (const [division, obs] of byDivision.entries()) {
    ...
    if (tier === 'distance_blind') {
      ...
      const mi = radiiForUnit(obs, MILE_EDGES, 'miles', tier, fallback.mi);
      const min = radiiForUnit(obs, MIN_EDGES, 'minutes', tier, fallback.min);
      ...
      for (const [bidderName, bObs] of byBidder.entries()) {
        const flags = bidderAutoFlags(bidderName, bObs, mi.competitive, mi.outer);
        if (!flags) continue;
